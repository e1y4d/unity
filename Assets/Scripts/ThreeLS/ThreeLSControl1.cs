...
public class ThreeLSControl1 : MonoBehaviour
{
    ...
    IEnumerator Init()
    {
        ...
        tPoseState = Animator.StringToHash("final_idle");
        yield return null;
    }
    ...
    void Update()
    {
        ...
        // Blend shapes
        UpdateBlendShapes();
        // Your script logic to determine if calibration is needed
        bool isCalibrationNeeded = audioInput.isPlaying;

        if (isCalibrationNeeded == false)
        {
            animator.Play(tPoseState);
        }
#if (UNITY_EDITOR)
